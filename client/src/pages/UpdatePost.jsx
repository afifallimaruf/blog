...
function UpdatePost() {
    ...
    const handleSubmit = async(e)=> {
        ...
        try {
            const res = await fetch(`http://localhost:4000/api/post/update-post/${currentUser._id}/${formData._id}`, {
                method: "PUT",
                credentials: "include",
    ...
    return (
        <div className="p-3 max-w-3xl mx-auto min-h-screen">
            <h3 className="text-center text-3xl my-7 font-semibold">Update a post</h3>
            <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
                <div className="flex flex-col gap-4 sm:flex-row justify-between">
                    <TextInput type="text" placeholder="Title" required id="title" className="flex-1" onChange={(e)=> setFormData({...formData, title: e.target.value})} defaultValue={formData.title} />
                    <Select onChange={(e)=> setFormData({...formData, category: e.target.value})} defaultValue={formData.category}>
